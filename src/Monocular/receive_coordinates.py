...
"""
Неактуальный скрипт для получения координат камеры от ORB-SLAM3 через порт zmq
"""
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=5556,
        help="Порт для приема координат (по умолчанию 5556)",
    )
        ...
        while running:
            try:
                ...
                if len(parts) == 4:
                    ...
                    logging.info(
                        f"Получены координаты: timestamp={timestamp}, x={x}, y={y}, z={z}"
                    )
                else:
