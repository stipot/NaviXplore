...
class SimpleDatasetRecorder:
    ...
    def close(self):
        ...
        logging.info(f"Записано {self.frame_count} кадров")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Запись монокулярного датасета из CARLA")
    parser.add_argument('--output', type=str, required=True, help='Путь для сохранения датасета')
    parser.add_argument('--duration', type=int, default=60, help='Длительность записи в секундах')
    ...
    logging.info(f"Запись датасета в директорию: {args.output}")
