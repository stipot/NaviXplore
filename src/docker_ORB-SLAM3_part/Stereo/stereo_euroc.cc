                ...
                vector<string> &vstrImageLeft, vector<string> &vstrImageRight, vector<double> &vTimeStamps)
{
    ...
    while(!fTimes.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            double t;
            ss >> t;
            // Формируем полный путь к изображению, включая .png
            string timestamp_str = to_string(static_cast<long long>(t));
            vstrImageLeft.push_back(strPathLeft + "/" + timestamp_str + ".png");
            vstrImageRight.push_back(strPathRight + "/" + timestamp_str + ".png");
