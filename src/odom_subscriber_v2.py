...
class OdomSubscriber(Node):
    ...
    def listener_callback(self, msg):
        ...
        latitude = msg.pose.pose.position.y / (EARTH_RADIUS * (math.pi / 180)) + latitude0
        longitude = msg.pose.pose.position.x / (EARTH_RADIUS * (math.pi / 180)) / \
                    (math.pi / 180 * latitude0) + longitude0
        lat_dir = 'N' if latitude >= 0 else 'S'
        lon_dir = 'E' if longitude >= 0 else 'W'
        nmea_sentence = generate_gprmc(abs(latitude), lat_dir, abs(longitude), lon_dir, 0.0, 0.0)
