# Инструкции по работе с частью проекта, связанной со сбором и использованием датасета из CARLA Simulator

## Установка симулятора городской среды CARLA Simulator
Весь проект требует наличия на системном диске не менее 360 ГБ свободной памяти для Unreal Engine 5 сборки и видеокарты уровня не ниже RTX 3070, но в документации рекомендуют использовать видеокарты с объёмом видеопамяти от 16 ГБ. Также, необходимо от 32 гигабайт оперативной памяти.
Репозиторий: https://github.com/carla-simulator/carla
Документация: https://carla-ue5.readthedocs.io/en/latest/tuto_first_steps/

Для установки нужно иметь зарегистрированный аккаунт GitHub. Его нужно связать с Epic Games по этой инструкции: https://www.unrealengine.com/en-US/ue-on-github, чтобы получить доступ к нужному репозиторию Unreal Engine 5.
- Установка:
```Shell
...
Перед запуском интерактивного режима управление камерой происходит через удержание правой кнопки мыши, после запуска - левой (но правая тоже работает). Перемещение в пространстве - на клавиши W, A, S, D. Увеличение скорости движения - с помощью прокрутки колёсика мыши: вперёд - увеличение скорости, назад - уменьшение.
